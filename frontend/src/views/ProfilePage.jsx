...
export function StudentProfile({ user, onNavigate, onLogout }) {
  ...
  useEffect(() => {
    ladeProfil();
    ladeStudienfortschritt();
  }, []);

  async function ladeStudienfortschritt() {
    try {
      setStatsLoading(true);
      const data = await getStudienfortschritt();

      const bestanden = data?.bestandenePruefungen?.length ?? 0;
      const nichtBestanden = data?.nichtBestandenePruefungen?.length ?? 0;
      const offen = data?.offenePruefungen?.length ?? 0;

      setStats({
        ects: data?.earnedEcts ?? 0,
        avgGrade: data?.averageGrade ?? 0,
        ...
        >
          ...
          >
            ...
            <div style={{ flex: 1, minWidth: 200 }}>
              ...
              >
                {form.studiengang || "—"} · Semester {form.semester || "—"} ·
                Matrikel-Nr. {form.matrikel || "—"}
              </div>
         ...
         {/* Stats */}
          <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 12, marginTop: 24 }}>
            <StatPill
              label="ECTS"
              value={statsLoading ? "…" : stats.ects}
              accent={t.accent}
            />
            <StatPill
              label="Ø Note"
              value={statsLoading ? "…" : Number(stats.avgGrade).toFixed(2)}
              accent="#a78bfa"
            />
            <StatPill
              label="Semester"
              value={`${form.semester || "0"}/6`}
              accent="#22c55e"
            />
            <StatPill
              label="Module"
              value={statsLoading ? "…" : stats.modulCount}
              accent="#f59e0b"
            />
          </div>
        </div>
