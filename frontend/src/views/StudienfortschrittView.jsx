...
import {
  getStudienfortschritt,
  downloadStudienfortschrittPdf,
} from "../api/authApi";
...
function GradeDistribution({ grades }) {
  const buckets = [
    {
      label: "1.0 - 1.5",
      count: grades.filter((g) => g >= 1.0 && g <= 1.5).length,
    },
    {
      label: "1.6 - 2.5",
      count: grades.filter((g) => g > 1.5 && g <= 2.5).length,
    },
    {
      label: "2.6 - 3.5",
      count: grades.filter((g) => g > 2.5 && g <= 3.5).length,
    },
    {
      label: "3.6 - 4.0",
      count: grades.filter((g) => g > 3.5 && g <= 4.0).length,
    },
  ...
  if (grades.length === 0) {
    return (
      <div style={{ fontSize: 12, color: "rgba(255,255,255,0.4)" }}>
        Noch keine benoteten Prüfungen.
      </div>
    );
  }
              ...
              >
                <div>
                  <div style={{ fontWeight: 700, fontSize: 14 }}>
                    {m.modulBezeichnung ?? "Unbekanntes Modul"}
                  </div>
                  <div
                    style={{
                      fontSize: 12,
...
export default function StudienfortschrittView({ onNavigate, user }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (mounted)
          setError(
            err.message || "Studienfortschritt konnte nicht geladen werden.",
          );
      } finally {
  ...
  const handleExportPdf = async () => {
    try {
      setExporting(true);
      setExportError("");
      await downloadStudienfortschrittPdf();
    } catch (err) {
      setExportError(err.message || "PDF konnte nicht exportiert werden.");
    } finally {
      setExporting(false);
    }
  };
          ...
          >
            <button
              ...
              style={{
                ...
                transition: "all 0.2s",
                boxShadow: exporting ? "none" : `0 4px 16px ${T.accentGlow}`,
              }}
            >
              {exporting ? (
                <>
            ...
            {exportError && (
              <div style={{ fontSize: 11, color: T.danger }}>{exportError}</div>
            )}
          </div>
        </div>
        ...
        {totalExams === 0 ? (
          ...
          <>
            ...
            >
              ...
              <RadialProgress
                pct={
                  totalExams ? Math.round((kpis.passed / totalExams) * 100) : 0
                }
                color={T.success}
                label="Prüfungen bestanden"
                value={`${kpis.passed}/${totalExams}`}
              />
              <RadialProgress
                pct={
                  totalExams ? Math.round((kpis.open / totalExams) * 100) : 0
                }
                color="#a78bfa"
                label="Offene Prüfungen"
                value={`${kpis.open}/${totalExams}`}
              />
              <div
                style={{
                  display: "flex",
            ...
            >
              <div>
                <ExamSection
                  ...
                  exams={bestanden}
                  color={T.success}
                  emptyText="Noch keine bestandenen Prüfungen."
                />
                <ExamSection
                  title="Offene Prüfungen"
                  exams={offen}
                  color="#a78bfa"
                  emptyText="Keine offenen Prüfungen."
                />
                <ExamSection
                  title="Nicht bestandene Prüfungen"
                  exams={nichtBestanden}
                  color={T.danger}
                  emptyText="Keine nicht bestandenen Prüfungen."
                />
              </div>
                    ...
                    <div key={item.label} style={{ marginBottom: 12 }}>
                      ...
                      >
                        <div
                          style={{
                            height: "100%",
                            width: `${totalExams ? Math.round((item.value / totalExams) * 100) : 0}%`,
